...
import { GitRulesService } from './services/GitRulesService.js'
import {
  AppUpdaterService,
  GitHubService,
  LogService,
...
logService.info(`GitRules Version ${logService.getAppVersion()}`)
...
async function setup(): Promise<void> {
  ...
  const gitRulesService = new GitRulesService(services)
  services.register('gitrules', gitRulesService)
...
async function serviceHandler(
  ...
  try {
    ...
    logService.info(`Service Invoked ${invokeProps.service}:`, invokeProps)
    logService.info(`Service Response ${invokeProps.service}:`, response)
    return response
  } catch (ex) {
    try {
