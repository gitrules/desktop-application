...
import { GitRulesService } from './GitRulesService.js'
...
export class ValidationService extends AbstractValidationService {
  ...
  private declare readonly govService: GitRulesService
  ...
  constructor({ services }: ValidationServiceOptions) {
    ...
    this.govService = this.services.load<GitRulesService>('gitrules')
  }
  ...
  private validateCommunityRepos = async (
    ...
    if (!communityRepoExists) {
      return {
        ok: false,
        statusCode: 400,
        error: `${community.projectUrl} is not a GitRules managed repo.`,
      }
    }
