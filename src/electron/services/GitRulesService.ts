import { runGitRules } from '@gitrules/js-client'
...
export class GitRulesService {
  ...
  private parse = <T>(stdout: string, cmd: string[]): T => {
    try {
      ...
      if (response.status === 'error') {
        throw new Error(
          `GitRules Error Response for command ${cmd.join(' ')}: ${stdout}`,
        )
      }
  ...
  public mustRun = async <T>(
    ...
    const run = async () => {
      const { stdout, stderr } = await runGitRules(...command)
      this.log.info('Running GitRules')
      this.log.info(`Command: ${command.join(' ')}`)
      this.log.info('GitRules Logs:', stderr)
      const output = this.parse<T>(stdout, command)
      this.log.info('GitRules Response:', output)
      return output
    }
    ...
    try {
      ...
      this.log.error(`Exception running GitRules`)
  ...
  public initId = async (): Promise<ServiceResponse<null>> => {
    ...
    if (user == null) {
      return {
        ok: false,
        statusCode: 401,
        error: `Unauthenticed. Failed to initialize GitRules identity repos. Please login.`,
      }
    }
    ...
    if (isPublicEmpty || isPrivateEmpty) {
      try {
        ...
        return {
          ok: false,
          statusCode: 500,
          error: `Failed to initialize GitRules identity repos. ${ex}`,
        }
      }
    }
